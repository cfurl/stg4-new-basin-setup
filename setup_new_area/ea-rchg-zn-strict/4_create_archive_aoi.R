# backfill_aoi_cum_precip_pipeline_schema_fast_schemafix.R
...
#   4) schema fix: stats/ytd year is character/string to match S3 pipeline
...
# ============================================================
AREA_ID <- "ea-rchg-zn-strict"   # <--- change ONLY this
...
expected_schemas <- list(
  ...
  stats_ytd = data.frame(
    ...
    # Source of truth from S3 pipeline: stats/ytd stores year as string/character,
    # while derived_ytd_precip stores year as int32/integer.
    expected_type = c("character", "character", "character", "integer", "integer", "integer", "numeric", "numeric", "numeric"),
    stringsAsFactors = FALSE
  )
)
...
coerce_ytd_stats_schema <- function(df) {
  df %>%
    dplyr::transmute(
      area_id = as.character(area_id),
      area_name = as.character(area_name),
      # Source of truth from S3 pipeline: stats/ytd year is string.
...
for (yr in years) {
  ...
  for (ii in seq_along(year_files)) {
    ...
    if (isTRUE(cfg$write_ytd_stats)) {
      ...
      ytd_stats <- cum_vol %>%
        mutate(area_id = cfg$area_id, year = as.character(yr),
